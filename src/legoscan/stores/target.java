...
public class target extends baseStore{
    ...
    private String baseUrl = "http://www.target.com/s?sortBy=PriceLow&facetedValue=d_item_type_all%3ABuilding%20Set&searchTerm=lego&Nao=??&og=lego";
...
//        }
    ...
    private void fetchData() {
        ...
        String currentUrl = baseUrl.replace("??", Integer.toString(0));
        try {
            Document doc = Jsoup.connect(currentUrl).get();
            System.out.println(doc.text());
            Elements options = doc.select("select#pagination").select("option");
    ...
    private void fetchDataByPrice(int lower, int higher) {
        ...
        try {
            Document doc = Jsoup.connect(currentUrl).get();
            System.out.println(doc.select("span#countMsg").text());
